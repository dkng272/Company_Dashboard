...
# %%
def get_project_basic_info_perplexity(project_name: str, perplexity_api_key: str) -> dict:
    """
    Get project information using Perplexity API with simplified prompt
    """
    
    if not perplexity_api_key:
        return {"error": "Perplexity API key not provided"}
    
    # Simplified prompt for Perplexity
    prompt = f"""
    Find information about the Vietnamese real estate project '{project_name}'.
    
    Please provide the following information in this exact format:
    
    Info: [Brief description of project location and developer]
    Average Selling Price: [price per m² in VND, numbers only]
    Total Units: [number of units, numbers only]
    Average Unit Size: [size per unit in m², numbers only]
    Gross Floor Area: [total floor area in m², numbers only]
    Construction Cost per sqm: [construction cost per m² in VND, numbers only]
    Land Area: [land area in m², numbers only]
    Land Cost per sqm: [land cost per m² in VND, numbers only]
    
    Important notes:
    - For numeric fields, provide ONLY numbers without commas or currency symbols
    - If exact data is not available, provide reasonable estimates based on similar Vietnamese projects
    - Focus on current market data from 2024-2025
    """
    ...
    try:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": "You are a Vietnamese real estate analyst. Provide accurate information about real estate projects in Vietnam. For numeric fields, provide only numbers without formatting."
                },
                {
                    "role": "user",
                    "content": prompt
    ...
    patterns = {
        ...
        "total_units": r"Total Units:\s*([0-9]+)",
        "average_unit_size": r"Average Unit Size:\s*([0-9]+)",
...
# ...existing code for save/load functions...

def main():
    st.title("Real Estate RNAV Calculator 3.2PM - Perplexity Edition")
                ...
                if isinstance(info, dict):
                    if "error" in info:
                        ...
                        if info.get("basic_info"):
                            st.info(f"📄 Project info found: {info['basic_info']}")
